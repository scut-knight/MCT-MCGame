...
/**
 * 在sqlite3中创建pattern table,rule table,state table,special pattern table,
 * special rule table
 */
- (id)init{
    if(self = [super init]){
...
- (BOOL)insertPattern:(NSString *)pattern withKey:(NSString *)key withPreState:(NSString *)state inTable:(NSString *)table{
    ...
    if (sqlite3_open([[self knowledgeBaseFilePath] UTF8String], &database) != SQLITE_OK) {
        ...
        NSString *insertPattern = [NSString stringWithFormat:@"INSERT OR REPLACE INTO %@(PRE_STATE, KEY, PATTERN) VALUES (?, ?, ?);",
                                                                                         table];
        sqlite3_stmt *stmt;
...
- (BOOL)insertRullOfMethod:(NSInteger)method withState:(NSString *)state ifContent:(NSString *)ifContent thenContent:(NSString *)thenContent inTable:(NSString *)table{
    ...
    if (sqlite3_open([[self knowledgeBaseFilePath] UTF8String], &database) != SQLITE_OK) {
        ...
        NSString *insertRule = [NSString stringWithFormat:@"INSERT OR REPLACE INTO %@(MEHOD, PRE_STATE, RULE_IF, RULE_THEN) VALUES (?, ?, ?, ?);",
                                                                                         table];
        sqlite3_stmt *stmt;
